# Path: database/core/data_paths.py
"""
Database Data Paths Manager

Automatic directory creation and validation for database module.
...
class DataPathsManager:
    """
    Manages directory creation and validation for database module.
    ...
    def ensure_all_directories(self) -> Dict:
        """
        Create all required directories for database module.
        
        Only creates essential directories:
        - Log directory
        - PostgreSQL data directory (if using local PostgreSQL)
        ...
        required_dirs = [
            self.config.get('db_log_dir'),
        ]
        
        # PostgreSQL data directory (only if configured locally)
        pg_data_dir = self.config.get('db_postgresql_data_dir')
        if pg_data_dir:
            required_dirs.append(pg_data_dir)
    ...
    def validate_paths(self) -> Dict:
        ...
        pg_data_dir = self.config.get('db_postgresql_data_dir')
        if pg_data_dir:
            results['postgresql_data_dir'] = self._validate_directory(
                pg_data_dir,
                writable=True
            )
