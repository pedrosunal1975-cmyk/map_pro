# Path: downloader/core/data_paths.py
"""
Downloader Data Paths Manager

Automatic directory creation and validation for downloader module.
...
class DataPathsManager:
    """
    Manages directory creation and validation for downloader module.
    ...
    def ensure_all_directories(self) -> dict:
        """
        Create all required directories for downloader module.
        
        Essential directories:
        - Log directory
        - Temp directory (for downloads)
        - Entities directory (for extracted filings)
        ...
        # Essential directories
        required_dirs = [
            self.config.get('downloader_log_dir'),
            self.config.get('downloader_temp_dir'),
            self.config.get('downloader_entities_dir'),
        ]
        
        # Optional cache directory
        cache_dir = self.config.get('downloader_cache_dir')
        if cache_dir:
    ...
    def validate_paths(self) -> dict:
        ...
        # Check temp directory (must be writable)
        temp_dir = self.config.get('downloader_temp_dir')
        results['temp_dir'] = self._validate_directory(temp_dir, writable=True)
        
        # Check entities directory (must be writable)
        entities_dir = self.config.get('downloader_entities_dir')
        results['entities_dir'] = self._validate_directory(entities_dir, writable=True)
    ...
    def create_filing_directory(
        ...
        Args:
            market: Market identifier (sec, esma, fca)
            company: Company name (as-is from database)
        ...
        """
        entities_root = self.config.get('downloader_entities_dir')
        
        filing_dir = entities_root / market / company / 'filings' / form_type / accession
    ...
    def cleanup_temp_directory(self, force: bool = False) -> dict:
        """
        Cleanup temporary directory.
        
        Args:
            force: If True, delete all temp files regardless of age
            ...
            for file_path in temp_dir.iterdir():
                if file_path.is_file():
                    try:
                        ...
                        # File deletion failed, increment counter and continue
                        failed_count += 1
        except Exception:
            # Directory iteration failed, return current counts
            pass
