...
class VerificationResult:
    ...
    Attributes:
        ...
        xbrl_calculation_results: Company XBRL calculation results (subset of vertical)
...
class VerificationCoordinator:
    ...
    def __init__(self, config: Optional[ConfigLoader] = None):
        ...
        # Initialize checkers
        # Note: HorizontalChecker now uses internal DecimalTolerance
        # Tolerances are handled internally per XBRL spec
        self.horizontal_checker = HorizontalChecker()

        # VerticalChecker still uses old interface - pass tolerance/rounding
    ...
    def verify_filing(self, filing: MappedFilingEntry) -> VerificationResult:
        ...
        try:
            ...
            calc_networks = self._load_calculation_linkbase_from_path(xbrl_path)
            self.logger.info(f"{LOG_OUTPUT} Loaded {len(calc_networks)} calculation networks")

            # Step 2b: Parse sign corrections from XBRL instance document
            # This is shared between horizontal and vertical checkers
            sign_handler = SignWeightHandler()
            sign_corrections_count = 0
            if xbrl_path:
                instance_file = self._find_instance_document(xbrl_path)
                if instance_file:
                    sign_corrections_count = sign_handler.parse_instance_document(instance_file)
                    self.logger.info(f"{LOG_OUTPUT} Parsed {sign_corrections_count} sign corrections")

            # Update checkers with shared sign handler
            self.horizontal_checker.sign_handler = sign_handler
            self.vertical_checker.sign_handler = sign_handler

            # Step 2c: Load formulas into registry (for XBRL-sourced verification)
            ...
            if self.enable_xbrl_verification:
                ...
                self.logger.info(
                    ...
                    f"{result.formula_registry_summary.get('company_trees', 0)} company calculation trees"
                )

            # Step 3: Run horizontal checks (sign handler already set)
            self.logger.info(f"{LOG_PROCESS} Running horizontal checks")
            result.horizontal_results = self.horizontal_checker.check_all(
                statements, calc_networks, filing_path=xbrl_path
            )

            # Step 4: Run vertical checks (sign handler already set)
            ...
            # Store company XBRL calculation results separately for detailed analysis
    ...
    def _load_calculation_linkbase_from_path(
        self,
        xbrl_path: Optional[Path]
    ) -> list[CalculationNetwork]:
        """Load calculation linkbase from a given XBRL path."""
        try:
            if xbrl_path:
                return self.xbrl_reader.read_calculation_linkbase(xbrl_path)
        except Exception as e:
    ...
    def _find_instance_document(self, filing_path: Path) -> Optional[Path]:
        ...
        # Priority order for instance documents (iXBRL first, then traditional)
        patterns = [
            ('*.htm', 'iXBRL HTML'),
            ('*.html', 'iXBRL HTML'),
            ('*.xhtml', 'iXBRL XHTML'),
        ...
        for pattern, doc_type in patterns:
            ...
            if files:
                ...
                instance_files = [
                    f for f in files
                    if not any(excl in f.name.lower() for excl in ['_cal.', '_pre.', '_def.', '_lab.', 'schema'])
                ]
                if instance_files:
                    # Prefer the largest file (usually the full instance document)
                    instance_file = max(instance_files, key=lambda f: f.stat().st_size)
                    self.logger.info(f"{LOG_INPUT} Found {doc_type} instance document: {instance_file.name}")
    ...
    def _load_formula_registry(
        ...
        # NOTE: Taxonomy formula loading has been removed
        # Standard taxonomy libraries don't contain company-specific extensions,
        # so taxonomy verification always fails on those concepts. Company XBRL
